...
class HotelManagementSystem:
    ...
    def checkout(self, room_id):
        ...
        if customer_info:
            ...
            print(f"Room {room_id} has been checked out by {customer_info['name']}. Total cost: {total_cost} (including services: {', '.join(customer_info['services'])}).")
            for reservation in self.reservations:
                if reservation['room_id'] == room_id:
                    self.reservations.remove(reservation)
                    break
        else:
            print("No customer found for this room.")
